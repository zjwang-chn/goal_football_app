...
轮次模拟页面：基于动态概率的回合制模拟（源自mini.py逻辑）
"""
...
def simulate_one_match(home_no_goal_probs, away_no_goal_probs):
    ...
    while True:
        ...
        if turn == "home":
            r = random.random()
            no_goal_p = home_no_goal_probs[home_goals]
            if r < no_goal_p:
                # 主队未进，客队补攻
                r2 = random.random()
                no_goal_p2 = away_no_goal_probs[away_goals]
                if r2 < no_goal_p2:
                    # 客队也未进 -> 结束
            ...
            r = random.random()
            no_goal_p = away_no_goal_probs[away_goals]
            if r < no_goal_p:
                r2 = random.random()
                no_goal_p2 = home_no_goal_probs[home_goals]
                if r2 < no_goal_p2:
                    round_history.append({
                        "轮次": rounds,
                        "进攻方": "客队 -> 主队",
...
def simulate_matches(home_no_goal_probs, away_no_goal_probs, n):
    """批量模拟"""
    results = []
    for _ in range(n):
...
if page == "首页":
    ...
    st.markdown('<p class="sub-header">📏 大小球 & 胜平负赔率 & 半场胜平负</p>', unsafe_allow_html=True)
    ...
    def calc_size_new(row):
        ...
        if z <= -0.5:
            return -1.0 * prob * uo
        elif -0.5 < z < -0.25:   # 实际只有 -0.5 < z <= -0.25 ?
            # 按用户描述：z=-0.25 时 -1/2 * 百分比 * (小球赔率+1)
            # 这里简化：若 z <= -0.25 且 > -0.5 统一用 -0.5 插值 ? 用户未说明连续区间，按最接近处理
            # 严谨起见：z ∈ (-0.5, -0.25) 按 -0.25 处理
    ...
    # 扩展的比分列表：原有20个 + 5个平局 + 新增的 5-0,5-1,...,4-5 以及其他
    base_scores = ["1-0","2-0","2-1","3-0","3-1","3-2","4-0","4-1","4-2","4-3",
                   "0-1","0-2","1-2","0-3","1-3","2-3","0-4","1-4","2-4","3-4"]
    draw_scores = ["0-0","1-1","2-2","3-3","4-4"]
    extra_scores = ["5-0","5-1","5-2","5-3","5-4","5-5","0-5","1-5","2-5","3-5","4-5"]
    fixed_scores = base_scores + draw_scores + extra_scores + ["其他"]

    # 赔率映射：原有h1-h10,a1-a10,o11-o16
    odds_mapping = {}
    for i, score in enumerate(base_scores[:10]):   # 主队赢的10个
        odds_mapping[score] = cs_odds[i] if i < len(cs_odds) else 0.0
    for i, score in enumerate(base_scores[10:]):   # 客队赢的10个
    ...
    if run_round_sim:
        with st.spinner(f"⏳ 正在进行 {n_round_sims:,} 次轮次模拟..."):
            df_results = simulate_matches(home_no_goal, away_no_goal, n_round_sims)
